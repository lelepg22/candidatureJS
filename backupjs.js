...
window.addEventListener("resize", () => {
  ...
  if (window.innerWidth <= 100) {
    alert(
      "Versão celular indisponível / Version mobile indisponible / Mobile version not available"
    );
  }
});
...
document.getElementById("hit").onloadedmetadata = function () {
  document
    .getElementById("hit")
    .insertAdjacentHTML(
      "afterend",
      `<audio preload="metadata" id="lose" src="assets/sounds/youlose.mp3"></audio>`
    );
  document.getElementById("lose").onloadedmetadata = document
    .getElementById("hit")
    .insertAdjacentHTML(
      "afterend",
      `<audio preload="metadata" id='plin' src='assets/sounds/plin.mp3'></audio>`
    );
  document.getElementById("plin").onloadedmetadata = document
    .getElementById("hit")
    .insertAdjacentHTML(
      "afterend",
      `<audio preload="metadata" id="fxkeyboard" src='assets/sounds/keyboard.wav'></audio>`
    );
  document.getElementById("fxkeyboard").onloadedmetadata = document
    .getElementById("hit")
    .insertAdjacentHTML(
      "afterend",
      `<audio preload="metadata" id='flame' src='assets/sounds/evilaugh.mp3'></audio>`
    );
  document.getElementById("flame").onloadedmetadata = document
    .getElementById("hit")
    .insertAdjacentHTML(
      "afterend",
      `<audio preload="metadata" id="fxsteps" src="assets/sounds/steps.mp3"></audio>`
    );

  setTimeout(function () {
    document.getElementById("fxsteps").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        ` <audio preload="metadata" id="plane" src="assets/sounds/street.mp3"></audio>`
      );
    document.getElementById("plane").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        ` <audio preload="metadata" id="round" src="assets/sounds/round.mp3"></audio>`
      );
    document.getElementById("round").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="fight" src="assets/sounds/fight.mp3"></audio>`
      );
    document.getElementById("fight").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="stage" src="assets/sounds/super.mp3"></audio>`
      );
    document.getElementById("stage").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        ` <audio preload="metadata" id="hadouk" src="assets/sounds/hadouken.mp3"></audio>`
      );
  }, 1000);

  setTimeout(function () {
    document.getElementById("hadouk").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="punch" src="assets/sounds/hit.wav"></audio>`
      );
    document.getElementById("punch").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="akum" src="assets/sounds/akuma.mp3"></audio>`
      );
    document.getElementById("akum").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="die" src="assets/sounds/deathryu.mp3"></audio>`
      );
    document.getElementById("die").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata" id="gameover" src="assets/sounds/gameover.mp3"></audio>`
      );
    document.getElementById("gameover").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata"  id="aplause" src="assets/sounds/bravo.mp3"></audio>`
      );
    document.getElementById("aplause").onloadedmetadata = document
      .getElementById("hit")
      .insertAdjacentHTML(
        "afterend",
        `<audio preload="metadata"  id="fim" src="assets/sounds/musicafim.mp3"></audio>`
      );
  }, 2000);
};
...
function mutePage() {
  ...
  if (localStorage.mute == "true") {
    ...
    document
      ...
      .insertAdjacentHTML(
        "afterend",
        `<i id="sound" onClick='mutePage()' class="fas fa-volume-up"></i>`
      );
  ...
  document
    ...
    .insertAdjacentHTML(
      "afterend",
      `<i id="sound" onClick='mutePage()' class="fas fa-volume-mute"></i>`
    );
  ...
  setTimeout(function () {
    if (document.getElementById("curtain").style.display != "none") {
      ...
      style.sheet.insertRule(
        "#recrut{position: absolute; right: 5%;height:52%; bottom: 9%; width: 24%;}"
      );
      document.getElementById("refuse").disabled = false;
      document.body.insertAdjacentHTML(
        "afterbegin",
        ` <img id="recrut" src="assets/img/UA0RXKs.gif"> <div id="lifebar1"><span id="health1"></span><h3 class="jouer">Candidat</h3></div><div id="lifebar2"><span id="health2"></span><h3 class="jouer">${localStorage.info}</h3></div><img id="ko" src="assets/img/KO.jpg"><p id="times">99</p> <img id="ryu" src="assets/img/Ryucvshados.gif">`
      );
      ...
      style.sheet.insertRule(
        "#ryu{position: absolute;height:60%; width: 50%; bottom: 8%; left: 7%;}"
      );
      style.sheet.insertRule(
        "#lifebar1{border: 3px solid white;background-color: yellow; width:32%; height:32px; position: absolute;left: 20%; top:17%;border-radius: 6px;}"
      );
      style.sheet.insertRule(
        "#lifebar2{border: 3px solid white;background-color: yellow; width:32%; height:32px; position: relative; left: 22%; bottom: -16vh;;border-radius: 6px;}"
      );
      style.sheet.insertRule(
        ".jouer{margin-top:40px; margin-left:1px; color:white;font-size:1.8rem;font-family:monospace; color:orange;text-overflow: ellipsis;white-space: nowrap; overflow: hidden;	}"
      );
      style.sheet.insertRule(
        "#ko{position: absolute;  top: 16%;  left: 52%;}"
      );
      style.sheet.insertRule(
        "#times{position: absolute;  top: 19.5%;  left: 53%; font-size:2.4rem;font-family:fantasy; color:orange;}"
      );
      ...
      document.body.style.backgroundImage =
        "url('assets/img/Super-Street-Fighter-II-Turbo-Arcade-Video-Game-1994-Capcom-Animated-Background-Stage-Guile-USA.gif')";
...
function start() {
  ...
  setTimeout(function () {
    document.querySelector("h1").insertAdjacentHTML(
      "afterend",
      `  <button onclick="createCandidat()" id="showCandidat">${txt0}
    
  </button>`
    );
    document.getElementById("showCandidat").insertAdjacentHTML(
      "afterend",
      `<div id="loadInit"><p class=" textLoad saving" style="animation: sortientree 5s forwards">${txt1}<span>.</span><span>.</span><span>.</span> </p>
...
function createCandidat() {
  ...
  document
    .getElementById("showCandidat")
    .insertAdjacentHTML(
      "afterend",
      `<div id='column' ><img id='photo' src='assets/img/fiddlecvphoto2.png'> </div><div id='card'><table><tr> <td>${txt0}</td> <td>Alexandre Gaglianone</td><tr> <td>${txt1}</td><td> 35</td> <tr><td>${txt2}</td> <td><strong>HTML, CSS/Sass, Bootstrap, JavaScript/Vue.Js, Node.Js, SQL/MySql, mongoDb, Git/GitHub.   </td></table> <div id='buttons'> <button id='accept' onclick='acceptIt()'>${txt3}</button><span id='mouse'> <button id='refuse' style='z-index:200;' onclick='refuseIt(0,0,0)'>${txt4} </button></div> </span></div>`
    );
}
...
function acceptIt() {
  ...
  document
    ...
    .insertAdjacentHTML(
      "afterend",
      `<ul id="acceptTxt"> <strong> <li> ${txt0}: ${txt1}768364537 </li> <li> email: alex.gaglianone@gmail.com </li>`
    );
...
function openFrame(x, y) {
  if (!document.getElementById("framed")) {
    if (x != 0) {
      return document
        ...
        .insertAdjacentHTML(
          "afterend",
          `<div id='framed'><a href="${x}" target="_blank" rel="noopener noreferrer"><i class='scaleText fas fa-globe-americas iconProj'"></i></a><a href="${y}" target="_blank" rel="noopener noreferrer"><i class='fab fa-github-square iconProj scaleText'"></i></a></div>`
        );
    }
    return document
      .querySelector("#exp")
      .insertAdjacentHTML(
        "afterend",
        `<div id='framed'><a href="${y}" target="_blank" rel="noopener noreferrer"><i class='fab fa-github-square iconProj scaleText' "></i></a></div>`
      );
  }
  ...
  return document
    ...
    .insertAdjacentHTML(
      "afterend",
      `<div id='framed'><a href="${y}" target="_blank" rel="noopener noreferrer"><i class='fab fa-github-square iconProj scaleText'"></i></div>`
    );
}
...
function refuseIt(x, y, z) {
    ...
    if (localStorage.timesClicked == 3) {
      ...
      setTimeout(function () {
        document
          ...
          .insertAdjacentHTML(
            "afterend",
            "<div class='buble' style='animation: tab1 1s forwards; border: solid 2px; width: 12px; height: 12px; border-radius: 20px; position: absolute; left: 10%; top:125px; z-index:	-1;'></div> "
          );
      }, 1200);
      setTimeout(function () {
        document
          ...
          .insertAdjacentHTML(
            "afterend",
            "<div class='buble' style='animation: tab1 1s forwards;border: solid 2px; width: 20px; height: 20px; border-radius: 20px; position: absolute; left: 10%;    top: 95px;z-index:	-1;'></div> "
          );
      }, 1500);
      ...
      setTimeout(function () {
        ...
        document.getElementById("photo").src =
          "assets/img/CREATOR-gd-jpeg-v1-0-using-IJG-JPEG-v80-default-quality.jpg";
      }, 1000);
      setTimeout(function () {
        document
          .getElementById("photo")
          .insertAdjacentHTML(
            "afterend",
            "<div id='ideaici'  style='border:0; position:absolute; background-image:url(assets/img/nuvemidea.png); background-size: contain;top:7px; left: 9.5%; height: 80px;'><img id='idea' src='assets/img/idea.png''></div> "
          );
      }, 3000);
      setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        document.body.insertAdjacentHTML(
          "afterbegin",
          `<div id='load'> <p class='saving'> ${txt0}<span>.</span><span>.</span><span>.</span></p> <div class='lds-facebook'><div></div><div></div><div></div></div> </div>`
        );
        ...
        style.sheet.insertRule(
          'h1{position:fixed; top:2px;background-color:white; text-transform: uppercase; text-align: center; line-height: 2; border-radius: 37px; width:55%; font-family: "Fredoka One", cursive; font-size:1.6em}'
        );
        style.sheet.insertRule(
          'button{background-color: #c2fbd7; border-radius: 100px;box-shadow: rgba(44, 187, 99, .2) 0 -25px 18px -14px inset,rgba(44, 187, 99, .15) 0 1px 2px,rgba(44, 187, 99, .15) 0 2px 4px,rgba(44, 187, 99, .15) 0 4px 8px,rgba(44, 187, 99, .15) 0 8px 16px,rgba(44, 187, 99, .15) 0 16px 32px; color: green; cursor: pointer; display: inline-block; font-family: CerebriSans-Regular,-apple-system,system-ui,Roboto,sans-serif; padding: 7px 20px;text-align: center; text-decoration: none; transition: all 250ms;border: 0; font-size: 16px; user-select: none; webkit-user-select: none; touch-action: manipulation; margin: 20px; margin-top: 13%;font-family: "Fredoka One", cursive;  }'
        );
        style.sheet.insertRule(
          "button:hover{box-shadow: rgba(44,187,99,.35) 0 -25px 18px -14px inset,rgba(44,187,99,.25) 0 1px 2px,rgba(44,187,99,.25) 0 2px 4px,rgba(44,187,99,.25) 0 4px 8px,rgba(44,187,99,.25) 0 8px 16px,rgba(44,187,99,.25) 0 16px 32px; transform: scale(1.05) rotate(-1deg);}"
        );

        style.sheet.insertRule(
          "i{color: white;font-size: 1.8rem;cursor: pointer;margin-right: 5px}"
        );
        style.sheet.insertRule(
          "#photo{position:unset;left: 36px;top:79px; border: 5px solid white !important;border-radius:25px;}"
        );
        style.sheet.insertRule(
          "#icons{width:100%;margin-top: 10px; margin-left:	5px;}"
        );
        style.sheet.insertRule(
          "#icons2{display:flex; flex-direction:column;width: 100%; margin-left: 5px;}"
        );
        style.sheet.insertRule("table{display:none}");
        style.sheet.insertRule(
          "#buttons{display: flex; position: fixed; bottom: 0%; right: 38%;}"
        );
        style.sheet.insertRule("button{margin: 0% 20px 20px !important;}");
        style.sheet.insertRule(
          "#card{margin-top:0; width:100%; position: relative;}"
        );
        style.sheet.insertRule(
          ".scaleIcon:hover{animation: scaleIcon 1s forwards}"
        );
        style.sheet.insertRule(
          "#column{display: flex; flex-direction: column;position: absolute; width: 21%; left: 10px; top: 18%;}"
        );
        style.sheet.insertRule(
          "#slider{display: flex;flex-direction: column;position: absolute;right: 20px;top: 48%;}"
        );
        ...
        style.sheet.insertRule(
          'input[type="radio"]:after{background-colord: red;}'
        );

        style.sheet.insertRule('label > input[type="radio"]{display: none;}');

        style.sheet.insertRule(
          'label > input[type="radio"] + *::before{ content: "";    display: inline-block; vertical-align: bottom; width: 0.7rem; height: 0.7rem; margin-right: 0.3rem; border-radius: 20%; border-style: double; border-width: 0.1rem;    border-color: white; margin-bottom: 5px;}'
        );

        style.sheet.insertRule(
          'label > input[type="radio"]:checked + *::before {background: radial-gradient(rgb(194, 251, 215) 0%, #225a14a3 100%, transparent 29%, transparent); border-color: white;}'
        );

        style.sheet.insertRule(
          ' label > input[type="radio"]:checked + * {color: rgb(194, 251, 215);}'
        );
        style.sheet.insertRule(
          ".scaleText:hover{animation: scaleText 1s forwards; cursor:pointer;}"
        );
        style.sheet.insertRule(".frames{border:0; border-radius:12px;}");
        style.sheet.insertRule(
          "#framed{animation: framesButton 1s forwards; width: 23%;background: radial-gradient(rgb(106 147 121) -7%, rgba(34, 90, 20, 0.64) 100%, transparent 0%, transparent);border-radius: 10px; height: 57px; display: flex; justify-content: space-evenly; align-items: center;  margin: auto;}"
        );

        /*  style.sheet.insertRule('{}')'*/

        if (window.innerWidth < 651) {
          document.getElementById("mouse").addEventListener("click", run);
        }
      ...
      function clickRun() {
        document
          ...
          .removeEventListener("mouseout", normal);
      ...
      function sad() {
        document.getElementById("photo").src =
          "assets/img/Photolab-634992194.jpg";
        document.getElementById("photo").style.animation =
          "100ms shake infinite alternate";
      }
      ...
      function run() {
        ...
        if (!document.getElementById("flame2")) {
          document
            .getElementById("photo")
            .insertAdjacentHTML("afterend", " <span id='flame2'></span>");
          ...
          style.sheet.insertRule(
            "body{background-image:url(assets/img/fire-15.gif); background-repeat:repeat; background-size: cover; background-attachment: fixed;}"
          );
        }
        ...
        setTimeout(function () {
          if (window.innerWidth > 650) {
            document
              .getElementById("mouse")
              .removeEventListener("mousemove", run);
          }
          if (window.innerWidth < 651) {
          ...
          if (!document.getElementById("ok")) {
            ...
            style.sheet.insertRule(
              "#ok{ position: absolute; top:32%; right: 25%; width: 45%; height:45%; background-color:white; border-radius:25px; animation: ok 1s forwards; display:flex; justify-content: center; align-items: center; font-size: 2rem;}"
            );
            style.sheet.insertRule(
              "#curtain{position:absolute; width:75%; height: 100%; background-color:black; z-index:2;    top: 18%;    left: 19%;  }"
            );
            style.sheet.insertRule("body{background-repeat:no-repeat}");
            style.sheet.insertRule(
              "#curtain{background-repeat:no-repeat; display:none;}"
            );

            document.body.insertAdjacentHTML(
              "afterend",
              '<div id="ok"> <h3 class="saving"> <span>O</span><span>K</span><span>.</span><span>.</span><span>.</span></h3>'
            );
            document.getElementById("photo").src =
              "assets/img/Photolab-634992194.jpg";
            setTimeout(function () {
              ...
              document.body.insertAdjacentHTML(
                "afterbegin",
                `<div style="height:unset;" id='curtain'><div id="vid" style="display:none;width:100%;height:0px;position:relative;padding-bottom:56.250%;"></div>`
              );
              if (window.innerWidth > 649) {
                document.body.style.backgroundImage =
                  "url('assets/img/opencloseyoutube.gif')";
              }
            }, 6000);
            setTimeout(function () {
              style.sheet.insertRule(
                ".tab1{animation: curtainClose 1.8s forwards}"
              );
              style.sheet.insertRule(
                "#tab2{animation: curtainClose 1.8s forwards}"
              );
              document.getElementById("ok").style.animation =
                "curtainClose 1.8s forwards";
            }, 8000);
            document.body.style.backgroundRepeat = "no-repeat";
            setTimeout(function () {
              document.body.style.backgroundImage = "url('')";
            ...
            setTimeout(function () {
              document.getElementById("photo").style.animation =
                "iconsComp 2s forwards";
              document.getElementById("photo").src = "assets/img/muro.JPG";
              style.sheet.insertRule(
                "#curtain{top: 17% !important; left: 19% !important;width: 61% !important;z-index:0 !important;}"
              );
              ...
              if (window.innerWidth > 649) {
                document
                  ...
                  .insertAdjacentHTML(
                    "afterbegin",
                    `<div id="show" style="width:100%;height:0px;position:relative;padding-bottom:56.250%;"><iframe src="https://streamable.com/e/xkls6n?autoplay=1&nocontrols=1&loop=0" frameborder="0" width="100%" height="unset" allowfullscreen allow="autoplay" style="width:100%;height:100%;position:absolute;left:0px;top:0px;overflow:hidden;"></iframe></div>`
                  );
          ...
          if (window.innerWidth > 649) { gofight(32000); }
          if (window.innerWidth < 650) { gofight(15000);}
        }, 9000);
      }
      ...
      setTimeout(function () {
        ...
        document
          ...
          .insertAdjacentHTML(
            "afterend",
            "<div id='slider'><label for='slide1'><input id='slide1' name='slider' type='radio' checked><span style='cursor:pointer;'></span></label><label for='slide2'><input  id='slide2'  type='radio' name='slider'><span style='cursor:pointer;'></span></label></div>"
          );
        document
          .getElementById("photo")
          .insertAdjacentHTML(
        ...
        if (localStorage.language == "english") {
          ...
          var txt7 =
            "Building a secure API for a restaurant review application";
        ...
        if (localStorage.language == "portugues") {
          ...
          var txt3 =
            "Transformação de uma maquete em um website com HTML & CSS";
          ...
          var txt7 =
            "Construindo um API seguro para uma aplicação de revisão de restaurante";
  ...
  if (localStorage.timesClicked >= 100) {
    ...
    setTimeout(function () {
      style.sheet.insertRule(
        "#health2{background-color: red; width: 2px; height: 32px;position: absolute; transform-origin: left; transform: scaleX(0); animation: lifehit 1s forwards;}"
      );
    }, 1600);
    ...
    setTimeout(function () {
      ...
      if (!document.getElementById("gifmaq")) {
        ...
        style.sheet.insertRule(
          "#gifmaq{width:82%; border-radius: 50px; border: 2px solid white; animation:fenetre 0.8s linear forwards;}"
        );
        ...
        document
          ...
          .insertAdjacentHTML(
            "afterend",
            `<div id="scene"><img id="gifmaq" src="assets/img/projet2gife07e7cddc8cc8473.gif"></div>`
          );
      }
    }, 450);
  ...
  if ((localStorage.timesClicked < 0) & (localStorage.timesClicked > -12)) {
    ...
    setTimeout(function () {
      document.getElementById("ryu").src = "assets/img/hadouken.gif";
      style.sheet.insertRule(
        "#form{width:50%;position:absolute;left:47%;top:50%;animation:fenetre 0.8s linear forwards;}"
      );

      style.sheet.insertRule(
        "#list{position: absolute; background-color: black; top: -12vh; left:-16vw; z-index: 1; display: flex; border-radius: 25px; border: 2px solid white; text-align: center;    width: 50px; height: 50px; align-items: center; justify-content: center;}"
      );
      style.sheet.insertRule(
        ".wbox{border: groove 2px white; padding: 6px; margin-left:25px; position: relative;  top: -10%;border-radius:5px;}"
      );
      style.sheet.insertRule(".ei{margin: 6px; width:30px;}");
      style.sheet.insertRule(
        "#weather{line-height: 29px; text-align: center; position: relative; bottom: 5px; border-radius: 6px;}"
      );
      style.sheet.insertRule(
        "#artist{line-height: 29px; text-align: center; position: relative; bottom: 5px; border-radius: 6px;}"
      );
      style.sheet.insertRule(
        "#codePostal{line-height: 29px; text-align: center; position: relative; bottom: 5px; border-radius: 6px;}"
      );
      style.sheet.insertRule(
        "#akuma{position: absolute;width: 53%;height:69%; bottom: 11px; animation: akuma 1s forwards}"
      );
      style.sheet.insertRule(
        "#paging{color:white; font-weight:heavy; cursor:pointer}"
      );

      style.sheet.insertRule(
        "#imgFrame{display:none;position:absolute; border-radius: 25px; right: -100px; bottom: -8px;border:solid; animation:tab1 1s forwards; height: 90px;}"
      );
      style.sheet.insertRule(
        "#imgArtist{border-radius: 25px;width: 90px; height: 90px;}"
      );

      style.sheet.insertRule(
        "#weatherBox{background-color: black; color: white; display: flex; flex-direction: column; min-width: 45%; line-height: 33px; height: 84px; justify-content: center; padding-left: 25px; border-radius: 20px; border: solid; position: absolute; top: 30%; left: 33%; animation: tab1 1s forwards;}"
      );
      style.sheet.insertRule(
        "#listArtists{display:flex; flex-direction:row;background-color: black; color: white; width: 65%; height: 33px; height: fit-content; justify-content: space-evenly; padding-left: 25px; border-radius: 20px; border: solid; position: absolute; top: 28%; left:23%; animation: tab1 1s forwards;padding-bottom:5px; min-height:80px;}"
      );
      ...
      document
        ...
        .insertAdjacentHTML(
          "afterend",
          `<div id="form"><div><input type="text" id="weather" placeholder='"${txt0}"'><button onClick="doWeather()" class="buttonForm"><i class="fas fa-cloud-sun-rain"></i></button></div><div><input type="text" id="codePostal" placeholder='${txt1} ex: "31000"'> <button onClick="doCodePostal()" class="buttonForm"><i class="fas fa-search-location"></i></button></div><div><input type="text" id="artist" placeholder='ex. ${txt2} : "Jimi Hendrix".'><button onClick="doArtist()" class="buttonForm"><i class="fas fa-music"></i></button></div></div>`
        );
    }, 450);
    setTimeout(function () {
      document.getElementById("ryu").src = "assets/img/Ryucvshados.gif";
    ...
    return setTimeout(function () {
      document.getElementById("health2").style.animation =
        "lifehit2 1s forwards";
    }, 1600);
  }
  if (localStorage.timesClicked == "z") {
    ...
    if (!document.getElementById("akuma")) {
      document
        ...
        .insertAdjacentHTML(
          "afterend",
          `<img id="akuma" src="assets/img/akuma-certto.gif"></img>`
        );
      setTimeout(function () {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        style.sheet.insertRule(
          "#health1{background-color: red; width: 2px; height: 32px;position: absolute; transform-origin: left; transform: scaleX(0); animation: lifehit3 1s forwards;}"
        );
      }, 5000);
      ...
      setTimeout(function () {
        ...
        style.sheet.insertRule(
          '.merci{font-family: "Fredoka One", cursive;top: 75%; animation: tab1 1s forwards; color:white; position:absolute;	}'
        );
        style.sheet.insertRule(
          "h3{animation: sortientree 10s forwards; color:white}"
        );
        style.sheet.insertRule(
          "#finalListe{list-style:circle; animation: sortientree 16s forwards;line-height: 41px; font-size: 1.4rem; margin-top:14px;}"
        );
        style.sheet.insertRule(
          "h2{animation: sorti 8s forwards; color:white;}"
        );
        style.sheet.insertRule(
          "#ici{position: absolute; text-align: center;  top: 73px;}"
        );

        style.sheet.insertRule(
          "#sortie{position:absolute;animation: sorti 8s forwards; top: 43%; text-align: center; width: 60%; line-height: 30px;}"
        );
        ...
        setTimeout(function () {
          ...
          if (window.innerWidth < 650) {
            document.getElementById("icons").style.position = "fixed";
            document.getElementById("icons").style.bottom = "8%";
            document.getElementById("icons").style.right = "36%";
          }
          document.querySelector("h1").insertAdjacentHTML(
            "afterend",
            `<div id="final"><ul id="finalListe">${txt0} :
...
<li style="opacity: 0; animation-delay:5s">${txt5}</li></ul>
        ...
        setTimeout(function () {
          ...
          if (localStorage.language == "francais") {
            ...
            var txt2 =
              "N'hésitez pas à partager, à commenter, à aimer, toute aide et retour est plus que bienvenue.";
          ...
          if (localStorage.language == "english") {
            ...
            var txt2 =
              "Feel free to share, comment, like, any help and feedback is more than welcome.";
          ...
          if (localStorage.language == "portugues") {
            ...
            var txt2 =
              "Sinta-se à vontade para compartilhar, comentar, qualquer ajuda e feedback é mais do que bem-vindo.";
...
${txt1}</h3>`
        ...
        setTimeout(function () {
          ...
          document.getElementById("ici").insertAdjacentHTML(
            "afterend",
            `<div id="sortie"><h3 style="color:red">${txt2}
</h3><h2>${txt3}</h2></div>`
          );
          setTimeout(function () {
            if (localStorage.language == "francais") {
            ...
            document.querySelector("#linkShare").insertAdjacentHTML(
              "beforeend",
              `
            ...
            setInterval(function () {
              let a = random();
              let b = random();
              if (document.getElementById("bye")) {
              ...
              document
                ...
                .insertAdjacentHTML(
                  "afterend",
                  `<p id="bye" style="animation:sorti 2s forwards; position: absolute; top:${a}%; right:${b}%; color:rgb(194, 251, 215); font-weight: bolder; font-family: 'Fredoka One', cursive; ">${txt4} </p>`
                );
            }, 5000);
          }, 5000);
          setTimeout(function () {
...
async function fetchWeather(city) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=ce16e5c8c3556db1888cd42b6cb63dc5`
  );
...
async function fetchArtist(artist) {
  const response = await fetch(
    `https://dev-assignment.ew.r.appspot.com/search?q=${artist}&offset=0`
  );
...
async function fetchArtists(artist, page) {
  const response = await fetch(
    `https://dev-assignment.ew.r.appspot.com/search?q=${artist}&offset=${page}&limit=3`
  );
...
async function fetchCodePostal(code) {
  const response = await fetch(
    `https://geo.api.gouv.fr/communes?codePostal=${code}`
  );
...
function artistInfo(params) {
  if (!document.getElementById("imgArtist")) {
    document
      .getElementById("imgFrame")
      .insertAdjacentHTML("beforeend", `<img id="imgArtist" src=""></img>`);
  }
...
async function paging(y) {
  ...
  document
    ...
    .insertAdjacentHTML(
      "afterend",
      `<div id="listArtists"><div id="imgFrame"><img id="imgArtist" src=""></img></div></div>`
    );
...
async function doArtist(x) {
  ...
  if (!artist.artists || artist.artists.total == 0) {
    document.getElementById("artist").style.animation =
      "buttonerror 2s forwards";
    return setTimeout(function () {
      document.getElementById("artist").style.animation = "";
    }, 2000);
  }
  ...
  if (artist.artists.total > 5) {
    ...
    document
      ...
      .insertAdjacentHTML(
        "beforeend",
        `<span id="list"><i class="paging fa fa-angle-left" onclick="paging('previous')"></i> <i style="margin:0;"  class="fa fa-angle-right paging" onclick="paging('next')"></i></span>`
      );
    let artists = await fetchArtists(x, 0);
    console.log(artists);
...
async function doCodePostal(x) {
  ...
  if (!codePostal[0]) {
    document.getElementById("codePostal").style.animation =
      "buttonerror 2s forwards";
    return setTimeout(function () {
      document.getElementById("codePostal").style.animation = "";
    }, 2000);
  ...
  if (document.getElementById("weatherBox")) {
    document.getElementById("weatherBox").remove();
    return document.getElementById("scene").insertAdjacentHTML(
      "afterend",
      `<div id="weatherBox"><p style="text-align:center;"><i style="position: relative;
